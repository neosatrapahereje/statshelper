    ...
    def __str__(self):
        ...
        for attr in self.__dict__.keys():
            if isinstance(self.__dict__[attr], (float)):
                kwargs.append(
                    "{name}={val:.3f}".format(name=attr, val=self.__dict__[attr])
                )
            elif isinstance(self.__dict__[attr], str):
                kwargs.append(
                    '{name}="{val}"'.format(name=attr, val=self.__dict__[attr])
                )
            else:
                kwargs.append("{name}={val}".format(name=attr, val=self.__dict__[attr]))

        out_str = "{test_name}({kwargs})".format(
            test_name=test_name, kwargs=",".join(kwargs)
        )
...
class MutipleLinearRegressionResult(object):
    def __init__(
        self, model_test, predictor_tests, model_params, standard_error, R2, R2_adj
    ):
    ...
    def pretty_print_results(self):
        ...
        for st, beta, se in zip(
            self.predictor_tests, self.model_params, self.standard_error
        ):
            # pvalue =
            line = ",".join(
                [
    ...
    def test(
        ...
        for ht in hypothesis_type:
            if ht not in ("lt", "gt", "equal"):
                raise ValueError(
                    'hypothesis typ should be "lt", "gt" or "equal", but given {0}'.format(
                        ht
                    )
                )

        if not (
            isinstance(null_hypothesis, (float, int))
            or isinstance(null_hypothesis, np.ndarray)
        ):
        ...
        if isinstance(null_hypothesis, np.ndarray):
            if len(null_hypothesis) != len(self.params):
                raise ValueError(
                    "The lenght of the null hypothesis must be the same as the parameters"
                )
        ...
        se, _ = _compute_se_variance_linear_model(
            x=self._add_intercept(ensure2d(x)), y=y, y_hat=y_hat
        )
...
def zheng_loh_model_selection(x, y, input_names=None, return_model_risks=False):
    ...
    lm = LinearModel(input_names=input_names, add_constant=True)
    # fit model
    ...
    se, var = _compute_se_variance_linear_model(x=x_intercept, y=y, y_hat=y_hat)
    ...
    for j in range(len(sort_idxs)):
        ...
        zl_lm = LinearModel(input_names=p_names, add_constant=add_constant)
    ...
    if return_model_risks:
        risk_info = dict(
            [(i, (r, zl.input_names)) for i, (r, zl) in enumerate(zip(risk, zl_models))]
        )
...
def example_LinearModel():
    ...
    statistics, y_hat = lm.test(
        x, y, hypothesis_type=["gt", "equal"], return_preds=True
    )
...
def example_zheng_loh():
    ...
    best_model = LinearModel(input_names=best_predictors, add_constant=add_constant)

    y_hat_best = best_model.fit_predict(x[:, bp_idxs], y)

    best_model2, bp_idxs2 = zheng_loh_model_selection(
        x, y, input_names=["x1", "x2", "x3", "x4", "x5", "x6"]
    )
...
def compute_F_statistic_and_pvalue(*args, repeated_measures=False):
    ...
    if repeated_measures:
        df_btwn, df_subject, df_error = __degree_of_freedom_(
            *args, repeated_measures=True
        )
    ...
    if repeated_measures:
        for arg in args:
            if len(arg) != len(args[0]):
                raise ValueError(
                    "For repeated measures ANOVA, all groups require "
...
def f_oneway(*args, **kwargs):
    ...
    if repeated_measures:
        (
            ...
            df_subject,
            df_error,
        ) = compute_F_statistic_and_pvalue(*args, repeated_measures=repeated_measures)
    else:
...
if __name__ == "__main__":
    ...
    ap = compute_F_statistic_and_pvalue(
